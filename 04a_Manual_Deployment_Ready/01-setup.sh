...
SECRET_NAME=${SECRET_NAME:-"storygen-google-api-key"}
...
if ! gcloud secrets describe "$SECRET_NAME" --project="$PROJECT_ID" &>/dev/null; then
    ...
    if [ -n "$GOOGLE_API_KEY" ] && [ "$GOOGLE_API_KEY" != "[REPLACE YOUR API KEY HERE]" ]; then
        ...
        echo -e "${RED}❌ GOOGLE_API_KEY not found or is placeholder in .env file${NC}"
        echo "Please add a valid GOOGLE_API_KEY to your .env file and run this script again."
        exit 1
    fi
else
